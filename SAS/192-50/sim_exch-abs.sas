...
/*  options mprint mlogic nocenter nodate nonumber;*/
  ...
  data individual;
    ...
    pijk = probnorm(intcpt + (&theta + &theta_j * abs(period - 3)) * xijk + &b * (period - 3));
  ...
  quit;
    ...
    select COALESCE(result_mix2.sim, result_mm2.sim) as sim,
           ...
           (case when COALESCE(result_mix2.varname, result_mm2.varname) = "Intercept" then &mu
                 ...
                 when COALESCE(result_mix2.varname, result_mm2.varname) = "trt*period1" then &theta_j * abs(1-3)
                 when COALESCE(result_mix2.varname, result_mm2.varname) = "trt*period2" then &theta_j * abs(2-3)
                 when COALESCE(result_mix2.varname, result_mm2.varname) = "trt*period3" then 0
                 when COALESCE(result_mix2.varname, result_mm2.varname) = "trt*period4" then &theta_j * abs(4-3)
                 when COALESCE(result_mix2.varname, result_mm2.varname) = "trt*period5" then &theta_j * abs(5-3) end) as true_mixed,
           (calculated true_mixed / sqrt(1 + &vc)) as true_mm,
...
%simulation(vc=0.2, theta_j=0,     design=dsdopt, c0=58, c1=94);
%simulation(vc=0.2, theta_j=-0.1,  design=dsdopt, c0=58, c1=94);
%simulation(vc=0.5, theta_j=0,     design=dsdopt, c0=58, c1=95);
%simulation(vc=0.5, theta_j=-0.1,  design=dsdopt, c0=58, c1=95);
%simulation(vc=0.8, theta_j=0,     design=dsdopt, c0=58, c1=96);
%simulation(vc=0.8, theta_j=-0.1,  design=dsdopt, c0=58, c1=96);

%simulation(vc=0.2, theta_j=-0.1,  design=usdopt, c0=21, c1=38, c2=37);
%simulation(vc=0.2, theta_j=-0.1,  design=usdopt, c0=21, c1=38, c2=37);
%simulation(vc=0.5, theta_j=-0.1,  design=usdopt, c0=20, c1=38, c2=38);
%simulation(vc=0.5, theta_j=-0.1,  design=usdopt, c0=20, c1=38, c2=38);
%simulation(vc=0.8, theta_j=0,     design=usdopt, c0=20, c1=38, c2=38);
%simulation(vc=0.8, theta_j=-0.1,  design=usdopt, c0=20, c1=38, c2=38);

%simulation(vc=0.2, theta_j=-0.1,  design=usdopt, c0=20, c1=38, c2=38);
%simulation(vc=0.2, theta_j=-0.1,  design=usdopt, c0=20, c1=38, c2=38);
%simulation(vc=0.5, theta_j=-0.1,  design=usdopt, c0=19, c1=39, c2=38);
%simulation(vc=0.5, theta_j=-0.1,  design=usdopt, c0=19, c1=39, c2=38);
%simulation(vc=0.8, theta_j=0,     design=usdopt, c0=19, c1=39, c2=38);
%simulation(vc=0.8, theta_j=-0.1,  design=usdopt, c0=19, c1=39, c2=38);
